...
export async function reserveSeats(showId, client, { seats, tempBookingId, vkUserId }) {
  ...
  try {
    const response = await client.postJson('/api/seats', body);
    if (response?.ok !== true) return { ok: false, error: { code: 'reserve_failed' } };
    return { ok: true };
  } catch (error) {
...
export async function submitBooking(showId, client, order, ctx = {}) {
  ...
  if (createResp?.ok === false || createResp?.error) {
    return { ok: false, stage: 'create', error: { code: 'create_failed' } };
  }
...
export function createCheckout({
  ...
  function renderShell() {
    root.innerHTML = `
      <section class="checkout">
        ...
        <p class="checkout__error" id="checkoutError" role="alert" hidden></p>
        <form class="checkout__form" id="checkoutForm" hidden>
  ...
  async function handleSubmit(event) {
    ...
    const contact = {
      ...
      vk: form.elements.namedItem('vk').value.trim(),
    };
    if (!contact.name || !contact.phone) {
      showError({ code: 'contact_required' }, 'Укажите имя и телефон.');
      return;
    }
    if (!form.reportValidity()) return;
    root.querySelector('#checkoutError').hidden = true;
    submitting = true;
    const pay = root.querySelector('#checkoutPay');
    pay.disabled = true;
    pay.textContent = 'Оформляем…';

    const reservation = await reserveSeats(showId, client, { seats: selected, tempBookingId, vkUserId });
    if (!reservation.ok) {
      submitting = false;
      renderCart();
      showError(reservation.error, 'Не удалось закрепить места. Обновите схему и попробуйте снова.');
      return;
    }
    ...
    if (!result.ok) {
      ...
      showError(result.error || { code: result.stage }, 'Не удалось открыть оплату. Заказ не подтверждён.');
      return;
    }
  ...
  function showError(error, message) {
    const alert = root.querySelector('#checkoutError');
